class UsersController < ApplicationController
  ...
  def activarinactivar
    if params[:act].to_s == 'ACTIVAR'
      Userspermiso.create(user_id: params[:id], objeto_id: 6, actualiza: 'S', elimina: 'S', crea: 'S')
      flash['success'] = "Activado perfil de prueba practica"
      redirect_to users_path
    elsif params[:act].to_s == 'INACTIVAR'
      Userspermiso.where(user_id: params[:id], objeto_id: 6).delete_all
      flash['success'] = "Inactivado perfil de prueba practica"
      redirect_to users_path
    end
  end
  ...
  def etapa
    ...
    if params[:etapa].to_s != ""
      if params[:cohorte_id].to_s != ""
        ActiveRecord::Base.connection.execute("update users set etapa = '#{params[:etapa]}', cohorte_id = #{params[:cohorte_id].to_i} where id = #{params[:id]}")
      else
